...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    return (
        ...
        plot1_path,
        plot2_path,
        None,  # placeholder for slope_more_extreme
        None,  # placeholder for intercept_extreme
        slopes,
        intercepts,
    )
...
def hypothesis_test():
    ...
    if test_type == '>':
        ...
        p_value = None  # Handle invalid test_type

    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    if p_value is not None and p_value <= 0.0001:
        fun_message = "Wow! You have encountered a very rare event!"
    else:
        fun_message = None
    ...
    plt.figure()
    plt.hist(simulated_stats, bins=30, alpha=0.5, color='blue', label='Simulated Statistics')
...
def confidence_interval():
    ...
    ci_lower = mean_estimate - t_critical * standard_error
    ci_upper = mean_estimate + t_critical * standard_error

    # TODO 16: Check if confidence interval includes true parameter
    includes_true = (ci_lower <= true_param) and (ci_upper >= true_param)
    includes_true_text = 'Yes' if includes_true else 'No'
    ...
    if includes_true:
        ...
        mean_color = 'red'
    plt.errorbar(S + 1, mean_estimate, yerr=[[mean_estimate - ci_lower], [ci_upper - mean_estimate]], fmt='o', color=mean_color, label=f'Mean Estimate: {mean_estimate:.2f}')
    ...
    return render_template(
        ...
        ci_lower=ci_lower,
        ci_upper=ci_upper,
        includes_true=includes_true_text,
        observed_stat=observed_stat,
        N=N,
        mu=mu,
        sigma2=sigma2,
